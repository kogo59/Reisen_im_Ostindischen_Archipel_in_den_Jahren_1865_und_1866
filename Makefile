BUILD = build
MAKEFILE = Makefile
OUTPUT_FILENAME = Reisen_im_Ostindischen_Archipel_in_den_Jahren_1865_und_1866
TITLE_NAME = "Reisen im Ostindischen Archipel in den Jahren 1865 und 1866"
...
$(BUILD)/pdf/$(OUTPUT_FILENAME).pdf: $(MAKEFILE) $(METADATA_PDF) $(CHAPTERS_HTML_PDF) $(CSS_FILE) $(IMAGES) $(COVER_IMAGE) $(METADATA_PDF) $(PREFACE_EPUB)
	mkdir -p $(BUILD)/pdf
	cp  *.css  $(IMAGES_FOLDER)
	cp  $(IMAGES_FOLDER)/Reisen_im_Ostindischen_Archipel_*.jpg .
	...
	rm  $(IMAGES_FOLDER)/*.css
	rm Reisen_im_Ostindischen_Archipel_*.jpg 
